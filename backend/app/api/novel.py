...
MVP 第一批：路由 + ChapterWriter 跑通；其余 Agent 后续批补，路由已预留。
"""
...
router = APIRouter(prefix="/api/novel", tags=["novel"], dependencies=[Depends(require_access)])
...
async def list_novels():
    """列出全部小说（供前端「切换新书」使用），按创建时间倒序。"""
    return await _mm().list_novels()
...
async def create_novel(payload: NovelCreate):
    novel_id = uuid.uuid4().hex[:12]
    await _mm().create_novel(novel_id, payload.model_dump())
...
async def generate_outline(payload: OutlineGenerateRequest):
    """由 PlotArchitect 生成分卷大纲（含全书总纲）并持久化到 novel.outline_json。"""
    mm = _mm()
    nov = await mm.get_novel(payload.novel_id)
    if not nov:
        raise HTTPException(status_code=404, detail="小说不存在")
...
async def update_world(novel_id: str, payload: WorldSettings):
    """作者手动编辑世界观（可查看可改）。"""
...
async def generate_characters(payload: CharacterGenerateRequest):
...
async def generate_chapter(payload: ChapterGenerateRequest):
    """由 Coordinator 编排：RAG 上下文 → ChapterWriter → ConflictEditor(四维评分，<24 自动重写≤2次)
    → 落库 → MemoryKeeper(更新角色/伏笔)。"""
    if not await _mm().get_novel(payload.novel_id):
        raise HTTPException(status_code=404, detail="小说不存在")
...
async def bootstrap_book(payload: BootstrapRequest):
...
async def review_chapter(payload: ChapterReviewRequest):
    mm = _mm()
    # 必须注入章节正文，否则审校无意义
...
async def update_chapter(novel_id: str, chapter_no: int, payload: ChapterUpdateRequest):
    ...
    mm = _mm()
    if not await mm.get_novel(novel_id):
        raise HTTPException(status_code=404, detail="小说不存在")
...
async def list_chapters(novel_id: str):
    if not await _mm().get_novel(novel_id):
        raise HTTPException(status_code=404, detail="小说不存在")
...
async def add_foreshadow(novel_id: str, payload: ForeshadowCreate):
    if not await _mm().get_novel(novel_id):
        raise HTTPException(status_code=404, detail="小说不存在")
...
async def update_characters(novel_id: str, characters: list[CharacterState]):
...
async def get_memory(novel_id: str):
    mm = _mm()
